...
alpha = 60*np.pi/180.
theta0 = np.arcsin(nc[i0]*np.sin(alpha/2))
...
theta0p = np.arcsin(np.sin(theta0)/nc) # 1st surface: theta_i is theta0 (index 1), theta_t is theta0p (index n)
theta1 = alpha - theta0p
theta1p = np.arcsin(np.sin(theta1)*nc) # 2nd surface: theta_i is theta1 (index n), theta_t is theta1p (index 1)
...
F1, F2, Fn = 4*2.54*1.e-2, 10.7*2.54*1.e-2, 12.0
