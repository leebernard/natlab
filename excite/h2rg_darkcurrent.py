...
temp_curve = np.linspace(40, 70, num=4)
...
# replace with log-normal distribution
# assume std dev is sqrt(N)
std = np.sqrt(dc_67_means)
# convert to logspace mean and std dev
mu_67 = np.log(dc_67_means**2/np.sqrt(dc_67_means**2 + std**2))
sigma_67 = np.sqrt((np.log(1 + std**2/dc_67_means**2)))
simulated_dc_67 = lognormal(mu_67, sigma_67)
...
bins = np.linspace(0, simulated_dc_67.max()/observation_duration, int(simulated_dc_67.max() + 1))
...
for n, temp_data in enumerate(temp_data_cube):
    ...
    # turn off hot pixels by setting threshold to 100 K
    # dc_means = generate_dc_means(temp_data, np.zeros(thresholds.shape)+100)
    std = np.sqrt(dc_means)
    mu = np.log(dc_means ** 2 / np.sqrt(dc_means ** 2 + std ** 2))
    sigma = np.sqrt((np.log(1 + std ** 2 / dc_means ** 2)))
    simulated_dc = lognormal(mu, sigma)
